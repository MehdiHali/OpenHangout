...
public class ChatServer {
	...
	@OnOpen
	public void open(Session session, @PathParam("roomID") int roomID) {
		System.out.println("OPENING A NEW WS CONNECTIONO");
	...
	public void handleMessage(String msg) {
		...
		if(!authenticated) {
			...
			try {
				...
				notifyMsg.setTimestamp(new Date().toString());
				// change the Session class name to UserSession
				UserSession userSession = new UserSession();
				System.out.println("USER SESSION ID IS "+this.session.getId());
				userSession.setId(this.session.getId());
...
//				this.session.getAsyncRemote().sendText("SERVER:: Welcome !");
	...
	private void broadCast(Message msg, int roomID) {
			 ...
			 */
			Set<String> thisRoomSessions = UserSessionDao.getSessionsIdByRoom(roomID);
			Set<Session> openSessions = session.getOpenSessions();
			openSessions.forEach((Session sess)->{
			if(thisRoomSessions.contains(sess.getId())&& !(sess.getAsyncRemote().equals(this.session.getAsyncRemote())))
			{
